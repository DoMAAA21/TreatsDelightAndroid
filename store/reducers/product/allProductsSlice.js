...
export const fetchAllStoreItems = createAsyncThunk('allProducts/fetchAllStoreItems', async (_, { dispatch }) => {
  try {
    dispatch(allProductsRequest());
    const token = await AsyncStorage.getItem('token');
    const user = await AsyncStorage.getItem('user');
    ...
    const { data } = await axios.get(`${BACKEND_URL}/api/v1/admin/store/${storeId}/all-store-items`,config);
    dispatch(allProductsSuccess(data.products));
    return data.products;
  } catch (error) {
    dispatch(allProductsFail(error.response.data.message))
